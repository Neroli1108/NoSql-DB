...
// Author:      Yunding LI, CST 4-187, Syracuse University   //
//                                                           //
///////////////////////////////////////////////////////////////
/*
 * Package Operations:
...
namespace Project2Starter
{
    ...
    public static class DisplayExtensions
    {
        ...
        //Create the function named PrintChild to output all Children what the user wants
        public static void PrintChild<Key, Value>(this DBEngine<Key, DBElement<Key, Value>> db, IEnumerable<Key> enumerable)
        {
            DBElement<Key, Value> elem = new DBElement<Key, Value>();
            foreach (var e in enumerable)
            {
                db.getValue(e, out elem);
                foreach (var m in elem.children)
                {
                    Console.WriteLine(m);
                }
            }
        }
        //Create the function named PrintEnumerable to output all Keys what the user wants
        public static void PrintEnumerable<Key, Value>(this DBEngine<Key, Value> db, IEnumerable<Key> enumerable)
        {
            foreach (var e in enumerable)
            { Console.WriteLine(e); }
            Console.WriteLine("==========");
        }
        //Create the function named PrintElem to output all Values what the user wants
        public static DBElement<Key,Value> PrintElem<Key, Value>(this DBEngine<Key, DBElement<Key, Value>> db, IEnumerable<Key> enumerable)
        {
            DBElement<Key, Value> elem = new DBElement<Key, Value>();
            foreach (var e in enumerable)
            {
                db.getValue(e, out elem);
                Console.WriteLine(elem.showElement());
