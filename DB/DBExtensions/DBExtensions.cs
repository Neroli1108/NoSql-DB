...
namespace Project2Starter
{
    ...
    public static class DBElementExtensions
    {
        ...
        public static string showDataBase<Key, Data>(this DBEngine<Key, DBElement<Key,Data>> DB)
    {
            StringBuilder accum = new StringBuilder();
            DBElement<Key, Data> elem = new DBElement<Key, Data>();
            foreach(var c in DB.Keys())
            {
                DB.getValue(c, out elem);
                accum.Append(string.Format(" \nKey = {0}", c));
                elem.showElement();
                accum.Append(elem.showElement());
            }
            return accum.ToString();
    }
    }
